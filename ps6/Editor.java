...
public class Editor extends JFrame {
	...
	public void drawSketch(Graphics g) {
		...
		if (curr != null) {
			curr.draw(g);  // if the curr shape exists then draw that
		}
	}
	...
	private void handlePress(Point p) {
		...
		else if (sketch.topMostShapId(p.x, p.y) != -1) {
			int ID = sketch.topMostShapId(p.x, p.y); //id of shape that contains the selected point
			curr = sketch.getShape(ID); // update current shape

			// for recolor mode, send recolor the shape request to the server
			if (curr != null && mode == Mode.RECOLOR) {
				comm.sendmsg("recolor " + ID + " " + color.getRGB());
			}
	...
	private void handleDrag(Point p) {
		...
		//for move mode, send request to server
		else if(mode == Mode.MOVE && movingId != -1 && moveFrom != null){
			int dx = p.x - moveFrom.x;		//change in x
			int dy = p.y - moveFrom.y;		//change in y
			comm.sendmsg("move " + movingId + " " + dx + " " + dy); // send message to the server
			moveFrom = p;				//change movefrom position by draggin
		}
	}
	...
	private void handleRelease() {
		...
		if(mode == Mode.DRAW && curr != null && drawFrom != null){
			...
			comm.sendmsg("add " + id + " " + shapeCount + " " + curr.toString());
