...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// TODO: YOUR CODE HERE
			if(tree != null) found = tree.findInCircle(mouseX, mouseY, mouseRadius);
		}
		else {
	...
	/**
	 * test2 case that runs when key 1 is pressed.
	 * 7 dots are created.
	 */
	private void test2() {
		tree = new PointQuadtree<Dot>(new Dot(150,250), 0,0,width,height); // start with A
		tree.insert(new Dot(98,45)); // B
		tree.insert(new Dot(666,666)); // C
		tree.insert(new Dot(84,311)); // D
		tree.insert(new Dot(17,18)); // E
		tree.insert(new Dot(1,2)); // F
		tree.insert(new Dot(5,5)); // G
		if(tree.size() == 7){
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		//drawing the blob
		g.fillOval((int)tree.getPoint().getX()-dotRadius, (int)tree.getPoint().getY()-dotRadius, mouseRadius, mouseRadius);
		//drawing horizontal line through the blob
		g.drawLine(tree.getX1(), (int)tree.getPoint().getY(), tree.getX2(), (int)tree.getPoint().getY());
		//drawing vertical line through the blob
		g.drawLine((int)tree.getPoint().getX(), tree.getY1(), (int)tree.getPoint().getX(), tree.getY2());

		// Recurse with children
		// TODO: YOUR CODE HERE

		if(tree.hasChild(1)) drawTree(g, tree.getChild(1), level + 1);//children in quad 1
		if(tree.hasChild(2)) drawTree(g, tree.getChild(2), level + 1);//children in quad 2
		if(tree.hasChild(3)) drawTree(g, tree.getChild(3), level + 1);//children in quad 3
		if(tree.hasChild(4)) drawTree(g, tree.getChild(4), level + 1);//children in quad 4
	}
