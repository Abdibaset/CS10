...
public class SketchServerCommunicator extends Thread {
	...
	public void run() {
		try {
			...
			// TODO: YOUR CODE HERE
			TreeMap<Integer, Shape> shapesMap = (TreeMap<Integer, Shape>) server.getSketch().getKeepTrackSketch();
			int shapeNum = server.getSketch().getShapeCount();

			for (Integer id : shapesMap.navigableKeySet()){
				// tell the new clients to add the shapes with the same ID as the current client
				send("add " + id + " " + shapeNum  + " " + shapesMap.get(id).toString());
			}
			...
			while ((line = in.readLine()) != null) {
				...
				Messages msg = new Messages(server.getSketch());
