...
public class ImageProcessingGUI0 extends DrawingGUI {
	...
	private boolean brushDownUp = false;		//checks if the brush is down
	/**
	...
	public void handleKeyPress(char op) {
		...
		else if (op == 'v'){ // this changes the brush to brush up
			brushDownUp = true;
		}
		else if(op == 'y'){ // method to revert the brush to brush down
			brushDownUp = false;
		}
		else {
	...
	public void handleMouseMotion(int x, int y){
		if(!brushDownUp) {    //if brush is down, the floralSquare is called, nothing on screen if down
			proc.floralSquare(x, y, 5);
		}
		repaint();
	}
