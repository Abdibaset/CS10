...
public class EditorOne extends JFrame {	
	...
	private void handleDrag(Point p) {
		...
		if(mode == Mode.DRAW){
			shape.setCorners((int) drawFrom.getX(), (int) drawFrom.getY(), p.x, p.y);
		}
		// In moving mode, shift the object and keep track of where next step is from
		if(mode == Mode.DRAW){
			shape.moveBy(p.x - ((int) moveFrom.getX()), p.y - ((int) moveFrom.getY()));
			moveFrom = p;
		}
		// Be sure to refresh the canvas (repaint) if the appearance has changed
		repaint();
	...
	private void handleRelease() {
		...
		if(mode == Mode.DRAW){ moveFrom = null;}
		// Be sure to refresh the canvas (repaint) if the appearance has changed
		repaint();
	}
